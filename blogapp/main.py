	...
	def get(self, post_id):
		if self.user:
			...
			if self.user.key().id() == post.owner.key().id():
				...
				error = "You cannot delete this post because it is not yours!"
				self.render('permalink.html',post = post, error = error, comments = post.comments)
				return
			self.redirect('/blog')
		else:
			self.redirect('/login')
			...
			if self.user.key().id() != post.owner.key().id():
				...
				for p in postlikes:
					if p.owner.name == self.user.name:
						...
						self.redirect('/blog/'+post_id)
						return
				like = Like(parent=blog_key(), post = post, owner = self.user, liked = True)
				like.put()
				self.redirect('/blog/'+post_id)
				return
			else:
...
class AddComment(BlogHandler):
	...
	def post(self, post_id):
		...
		author = self.user
		c = Comment(parent = blog_key(), content = content, author= author, post = post)
		c.put()
		self.redirect('/blog/'+post_id)
	...
	def get(self, comment_id):
		if self.user:
			...
			if self.user.key().id() == comment.author.key().id():
				...
				error = "You cannot delete this comment because it is not yours!"
				self.render('permalink.html',post = post, error = error, comments = post.comments)
		else:
			self.redirect('/login')
